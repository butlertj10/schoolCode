...
void UpdateBall()
{
    if(ball.isLocked) //If the ball is locked, position it on top of the paddle and centered on the X axis
    {
    ...
    else
    {
        ...
        if(RectsOverlap(ball.rect, player.rect))
        {
            ...
            //Make the X velocity the distance between the paddle's center and the ball's center divided by 5
        ...
        else  //If not, check if it hit any blocks while moving on the Y-Axis
        {
            for(int i = 0; i < BLOCK_COLUMNS*BLOCK_ROWS; i++)
            {
                if(blocks[i].alive && RectsOverlap(ball.rect, blocks[i].rect))
                {
                    // Changes ball velocity, removes 3 block when red is hit, adds 3 to score
                    if((blocks[i].frame) % 4 == 0)
                    {
                        ball.rect.y -= ball.yVel;
                        ball.yVel *= -1;
                        blocks[i-5].alive = false;
                        blocks[i+5].alive = false;
                        blocks[i].alive = false;
                        Score += 3;
                    ...
                    // Adds life if Score is a multiple of 10
                    if(Score % 10 == 0)
                    player.lives += 1;
                }
            }
...
void DrawGame()
{
    ...
    DrawText(Backbuffer, scoreCount, 645, 180, GameFont, 255, 255, 255);
    DrawText(Backbuffer, blocksText, 645, 140, GameFont, 255, 255, 255);
    DrawText(Backbuffer, livesText, 645, 160, GameFont, 255, 255, 255);
}
