...
bool PauseState::init(GameState* r, Input* i)
{
    ...
    if(!continueButtonImage.load("graphics/menus/pause/continue_button.bmp"))
        return false;

    if(!exitButtonImage.load("graphics/menus/pause/exit_button.bmp"))
        return false;

    if(!overlayImage.load("graphics/menus/pause/overlay.bmp"))
        return false;
