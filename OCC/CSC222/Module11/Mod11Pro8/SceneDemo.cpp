...
bool SceneDemo::init()
{
    if(!initSystem("Mod 11 Pro 8", 800, 600, false))
        return false;

    if(!map.load("graphics/map.map", "graphics/tiles.bmp"))
        return false;

    if(!playerImage.load("graphics/playerImage.bmp", 20, 22))  //load the sprite bitmap
        return false;

    if(!background.load("graphics/background.bmp"))  //load the sprite background
        return false;

    if (!factory.init(&map))
        return false;

   // player = new MapNode(ENTITY_PLAYER, 50, 72, 20, 22, true, &map, &playerImage);
    player = new Player(ENTITY_PLAYER, PLAYER_X, PLAYER_Y, PLAYER_WIDTH, PLAYER_HEIGHT, true, &map, &playerImage);

    scene.addNode(player);
    scene.addNode (factory.create(ENTITY_SLUG, 200, 850));
